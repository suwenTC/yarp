...
while [ $i -le 100 ]
	  ...
	  bash $PWD/run_query.sh "SELECT * from orderdetails AS od RIGHT JOIN orders AS o ON od.orderNumber = o.orderNumber WHERE od.quantityOrdered > 0 ORDER BY od.quantityOrdered DESC;"
	  ...
	  bash $PWD/run_query.sh "SELECT * from orderdetails AS od RIGHT JOIN products AS p ON od.productCode = p.productCode WHERE od.quantityOrdered > 0 ORDER BY od.quantityOrdered DESC"
